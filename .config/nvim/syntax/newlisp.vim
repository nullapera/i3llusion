...
"
" * based on newLISP 10.7.6 release
...
" This keywords list is based on newLISP v.10.7.6 build primes.h file
" XXX Don't forget to remove colon ":" and escape vertical bar "|"

syn keyword newlispOperator ! != $ % & * + ++ - -- / < << <= = > >= >> ^ ~ \|
...
syn keyword newlispFunction def-new default delete delete-file delete-url
...
syn keyword newlispFunction get-url global global? history if if-not ifft import inc index inf? int integer
...
syn keyword newlispFunction sinh sleep slice sort source spawn sqrt ssq starts-with stats string string?
...
syn keyword newlispDefine   fn define define-macro lambda lambda-macro macro
...
hi def link newlispSymbol Normal
hi def link newlispSymbolString Normal
hi def link newlispQuote Type
hi def link newlispQuotedSymbol Normal
...
hi def link newlispOperator Type
hi def link newlispFunction Statement
hi def link newlispVariable Type
hi def link newlispDefine Define
